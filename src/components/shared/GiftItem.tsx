...
const GiftItem: FC<GiftItemProps> = memo(({ gift, variant = 'unbox', className }) => {
  const isGift = useMemo(() => variant === 'gift', [variant])
  ...
  return (
    ...
    >
      <p
        className={classNames(
          isGift ? 'text-blue-main' : 'text-pink-main',
          'text-[20px]/[30px] transition-300 group-hover:text-gray-1'
        )}
      >

        {/* {gift && isGift ? gift.giftName : 'Món quà của bạn là ?'} */}
      </p>
      <div className={classNames('z-10 size-full flex-1')}>
        <img
          src={gift && isGift ? gift.giftImageUrl : images.unbox}
          alt={gift && isGift ? gift.giftName : 'unbox'}
